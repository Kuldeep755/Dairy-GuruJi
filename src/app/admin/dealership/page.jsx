...
export default async function AdminDealershipPage() {
  ...
  try {
    const response = await fetch(backendApiUrl("/api/forms/admin/dealer?limit=100"), {
    ...
    if (!response.ok) {
      fetchError = data?.error || "Unable to load dealer applications.";
    } else {
      applications = Array.isArray(data?.applications) ? data.applications : [];
      total = typeof data?.total === "number" ? data.total : applications.length;
    }
  } catch {
    fetchError = "Unable to load dealer applications.";
  }

  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
      <header className="rounded-3xl border border-[#d6e4e1] bg-white/90 p-6 shadow-sm backdrop-blur">
        <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#0f766e]">Dealership Form Data</p>
        <h1 className="mt-2 text-3xl font-black tracking-tight text-[#0f172a]">Dealership Applications</h1>
        <p className="mt-2 text-sm text-slate-600">All submitted dealership inquiry forms.</p>
        <p className="mt-3 inline-flex rounded-lg bg-[#ecf7f4] px-3 py-1 text-sm font-semibold text-[#0f766e]">Total Applications: {total}</p>
      </header>
      ...
      <section className="overflow-hidden rounded-2xl border border-[#d8e5e2] bg-white shadow-sm">
        {fetchError ? (
          ...
          <p className="px-6 py-8 text-sm font-medium text-slate-500">No dealership applications yet.</p>
        ) : (
            ...
            <table className="w-full min-w-[1000px] text-left text-sm">
              <thead>
                <tr className="bg-slate-50/80 border-b border-slate-100">
                  ...
                  <th className="px-5 py-3 text-xs font-bold uppercase tracking-wider text-slate-500">Territory</th>
                  <th className="px-5 py-3 text-xs font-bold uppercase tracking-wider text-slate-500">Experience</th>
                  <th className="px-5 py-3 text-xs font-bold uppercase tracking-wider text-slate-500">Investment</th>
              ...
              <tbody>
                {applications.map((row) => (
                  <tr key={row.id} className="border-b border-slate-100 align-top">
                    ...
                    <td className="px-5 py-4 text-slate-600">{row.territory || "-"}</td>
                    <td className="px-5 py-4 text-slate-600">{row.experience || "-"}</td>
                    <td className="px-5 py-4 text-slate-600">{row.investment || "-"}</td>
