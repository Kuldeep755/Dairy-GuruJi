...
export default async function ProductDetailPage({ params }) {
  ...
  return (
    <div className="relative min-h-screen bg-[#fafaf8] pt-32 pb-20 font-sans">
      {/* Decorative Elements */}
      <div className="absolute top-0 right-0 w-1/3 h-1/4 bg-primary/5 blur-[120px] rounded-full -z-10" />
      <div className="absolute bottom-10 left-10 w-1/4 h-1/4 bg-secondary/10 blur-[100px] rounded-full -z-10" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
          ...
          <div className="lg:col-span-5 sticky top-32 space-y-8">
            <div className="relative group">
              <div className="absolute inset-0 bg-primary/20 rounded-[3rem] blur-[60px] opacity-50 group-hover:opacity-80 transition-opacity" />
              <div className="relative bg-white rounded-[3rem] p-8 md:p-12 shadow-2xl border border-primary/5 overflow-hidden">
                <div className="absolute -top-12 -right-12 w-48 h-48 bg-secondary/5 rounded-full" />
                <Image
                  src={product.image}
                  alt={product.name}
                  width={600}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Link
                href="/contact"
                className="flex items-center justify-center bg-primary text-white text-lg font-black py-5 rounded-2xl shadow-xl shadow-primary/20 hover:bg-text-dark hover:-translate-y-1 transition-all duration-300"
              >
                ENQUIRE NOW
              </Link>
              <a
                ...
                className="flex items-center justify-center bg-[#25D366] text-white text-lg font-black py-5 rounded-2xl shadow-xl shadow-[#25D366]/20 hover:bg-[#128C7E] hover:-translate-y-1 transition-all duration-300 gap-3"
              >
          ...
          <div className="lg:col-span-7 space-y-12">
            ...
            <div className="space-y-4">
              ...
              <h1 className="text-5xl md:text-7xl font-black text-text-dark leading-[1.05] tracking-tight">
                {product.name}
              </h1>
              <p className="text-xl md:text-2xl text-text-dark/70 font-medium leading-relaxed max-w-2xl">
                {product.description}
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              {composition && (
                <div className="bg-white rounded-[2.5rem] p-8 md:p-10 border border-primary/10 shadow-xl shadow-primary/5 relative overflow-hidden">
                  ...
                  <div className="space-y-4">
                    {composition.map((item, index) => (
                      <div key={index} className="flex justify-between items-center py-2 border-b border-primary/5 last:border-0 hover:bg-primary/5 px-2 rounded-lg transition-colors">
                        <span className="text-xs font-black uppercase tracking-wider text-text-dark/60">{item.label}</span>
                        <span className="text-sm font-black text-text-dark">{item.value}</span>
                      </div>
                    ))}
                  </div>
                </div>
                    ...
                    <div className="space-y-4">
                      {product.benefitsHindi.map((benefit, index) => (
                        <div key={index} className="flex gap-4 items-start group">
                          ...
                          <span className="text-lg font-bold text-text-dark leading-snug">
                            {benefit}
                          </span>
                        </div>
                      ))}
            ...
            {(product.feedingInstructions || product.packaging) && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch">
                {product.feedingInstructions && (
                  <div className="bg-[#fdfaf2] rounded-[2.5rem] p-8 md:p-10 border border-secondary/20 h-full">
                    <h3 className="text-text-dark/60 text-[10px] font-black uppercase tracking-[0.25em] mb-8 flex items-center gap-3">
                      ...
                      {product.feedingInstructions.items ? (
                        ...
                        <>
                          {product.feedingInstructions.large && (
                            <div className="flex flex-col gap-1 p-4 rounded-2xl bg-white/60 border border-secondary/10 shadow-sm">
                              <span className="text-xs font-black text-text-dark/40 uppercase tracking-widest">बड़े पशु</span>
                              <span className="text-lg font-black text-text-dark">{product.feedingInstructions.large}</span>
                            </div>
                          )}
                          {product.feedingInstructions.small && (
                            <div className="flex flex-col gap-1 p-4 rounded-2xl bg-white/60 border border-secondary/10 shadow-sm">
                              <span className="text-xs font-black text-text-dark/40 uppercase tracking-widest">छोटे पशु</span>
                              <span className="text-lg font-black text-text-dark">{product.feedingInstructions.small}</span>
                            </div>
                          )}
                        </>
                      )}
                ...
                {product.packaging && (
                  <div className="bg-primary rounded-[2.5rem] p-10 flex flex-col items-center justify-center text-center text-white relative overflow-hidden group shadow-2xl shadow-primary/20">
                    <div className="absolute top-0 right-0 w-full h-full bg-gradient-to-br from-white/10 to-transparent pointer-events-none" />
                    <Package className="w-12 h-12 mb-6 text-secondary animate-pulse" />
                    ...
                    <p className="text-2xl md:text-3xl font-black leading-tight" 
                       dangerouslySetInnerHTML={{ __html: product.packaging.replace(' : ', '<br/>').replace(' में उपलब्ध', '') }} 
                    />
                    <div className="mt-6 px-4 py-2 rounded-full border border-white/20 text-[10px] font-black uppercase tracking-widest">
                      Eco-Friendly Pack
                    </div>
                  </div>
                )}
            ...
            {product.specialNote && (
              <div className="relative p-1 rounded-[2rem] bg-gradient-to-r from-primary via-secondary to-primary shadow-2xl overflow-hidden group">
                <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] opacity-10" />
                <div className="relative bg-text-dark rounded-[1.9rem] p-8 md:p-12 overflow-hidden flex flex-col md:flex-row items-center gap-8 justify-between">
                  ...
                  <div className="relative z-10 flex-1">
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <span className="text-secondary text-xs font-black uppercase tracking-[0.3em]">Australian Standard</span>
                    </div>
                    <p className="text-white text-xl md:text-2xl font-black leading-relaxed italic">
                      "{product.specialNote}"
                    </p>
                  </div>
                  <div className="relative z-10 flex-shrink-0">
                    <div className="w-24 h-24 rounded-full border-2 border-secondary/30 flex items-center justify-center p-2">
                       <div className="w-full h-full rounded-full bg-secondary flex items-center justify-center">
                          <svg className="w-10 h-10 text-text-dark" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                ...
                <div key={idx} className="bg-white rounded-3xl p-8 border border-primary/5 shadow-sm hover:shadow-md transition-shadow">
                  <h4 className="text-primary font-black uppercase text-[10px] tracking-widest mb-6 flex items-center gap-3">
                    <section.icon className="w-4 h-4" /> {section.title}
                  </h4>
                  <ul className="space-y-3">
                    {section.items.map((item, i) => (
                      <li key={i} className="text-sm font-bold text-text-dark/60 leading-snug flex items-start gap-2">
